...
```
Ce choix est également cohérent avec les principes de sémiologie graphique vus en cours. Les équipes constituent une variable catégorielle tandis que le nombre de buts est une variable quantitative. Le diagramme en barres est particulièrement adapté à cette situation car il utilise la longueur comme canal visuel principal, ce qui permet une comparaison précise des valeurs entre catégories.
...
L'application est organisée autour de trois visualisations interactives accessibles depuis un menu latéral. Plusieurs filtres permettent de personnaliser l'analyse selon les besoins de l'utilisateur.
...
Ce graphique interactif présente les équipes recevant le plus de cartons en moyenne. L'utilisateur peut ajuster le nombre d'équipes affichées grâce à un curseur dédié.
...
Cette visualisation met en évidence la relation entre le nombre total de tirs réalisés lors d'un match et le nombre total de buts inscrits. Les filtres permettent d'explorer différents contextes de jeu.
